...
| GitHub → GitLab | Push na `main` spustí `.github/workflows/sync-to-gitlab.yml` |
| Build | GitLab CI na runneru s tagem `myskin` (viz `.gitlab-ci.yml`) |

```bash
docker pull registry.gitlab.catania-service.cz/catania_dev/myskin:latest
docker compose up -d --force-recreate   # po aktualizaci image v compose
```
